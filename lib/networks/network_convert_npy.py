...
class Network(object):
    ...
    def load(self, data_path, session, saver, ignore_missing=False):
        ...
        print "restore " + data_path

        keys = ['conv1_1', 'conv1_2', 'conv2_1', 'conv2_2', 'conv3_1', 'conv3_2', 'conv3_3',
        'conv4_1', 'conv4_2', 'conv4_3', 'conv5_1', 'conv5_2', 'conv5_3', 'rpn_conv/3x3',
        'rpn_cls_score', 'rpn_bbox_pred', 'conv6_1', 'conv6_2', 'conv6_3', 'fc6', 'fc7', 
        'cls_score', 'bbox_pred', 'up_1', 'mask_out']
        subkeys = ['weights','biases']
        # for up_1, the subkey is up_kernel
        ...
        for key in keys:
		    ...
		    sub_dict = {}
                    try:
                        var = tf.get_variable('up_kernel')
                        sub_dict['up_kernel'] = var.eval()
                        print "read " + key + "/" + "up_kernel"
                    except ValueError:
                        print "cannot find " + key + "/" + subkey
		    ...
		    sub_dict = {}
			    ...
			    sub_dict[subkey] = var.eval()
                            print "read " + key + "/" + subkey
                        except ValueError:
                            print "cannot find " + key + "/" + subkey
		    data_dict[key] = sub_dict
        data_name = "mask_rcnn_iter_135000.npy"
        np.save("/home/chsiyuan/Documents/2017Challenge/data/pretrain_model/" + data_name, data_dict)
